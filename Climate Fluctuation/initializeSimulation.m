...
A_max = 0 * ones(I, 1); % maximum velocity adjustment for optimal advective dispersal (coefficient of the optimal dispersal term)
...
initialPopulation.density = 0.8 * sech( abs(x - center) / radius )' + epsilon;
center_index_x = dsearchn(x', center); 
initialPopulation.trait_mean = 0.7 * ( Q_opt - Q_opt(center_index_x) ) + Q_opt(center_index_x);
initialPopulation.trait_variance = 1 * ones(I, 1);

% %---Initialize with the nominal solution at t=4
% load('Data\sol_A_max_0.mat', 'population' ) 
% %load('Data\sol_A_max_0_gradient_1.mat', 'population' ) 
