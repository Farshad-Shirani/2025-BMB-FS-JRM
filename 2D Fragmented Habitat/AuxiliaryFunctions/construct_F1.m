...
sigma_squared = D(1,1);
dQ_tilde_here = dQ_tilde(:,:,1); %1st component of dQ_tilde, since we are calulating F1
...
Ad_minusTwo = reflect( Ad, [2 0] ); % Ad(i-2,j)  reflect also applies the reflecting boundary condition
Ad_minusOne = reflect( Ad, [1 0] ); % Ad(i-1,j)  
Ad_plusOne = reflect( Ad, [-1 0] ); % Ad(i+1,j)  
Ad_plusTwo = reflect( Ad, [-2 0] ); % Ad(i+2,j)

AQd_minusTwo = reflect( AQd, [2 0] ); % AQd(i-2,j)  reflect also applies the reflecting boundary condition
AQd_minusOne = reflect( AQd, [1 0] ); % AQd(i-1,j)  
AQd_plusOne = reflect( AQd, [-1 0] ); % AQd(i+1,j)  
AQd_plusTwo = reflect( AQd, [-2 0] ); % AQd(i+2,j)
%-------------------------------------------------------

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%--- The code below is the same as the code of construct_F1.m 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
